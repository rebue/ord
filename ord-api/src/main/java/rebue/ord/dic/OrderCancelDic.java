...
/**
 * 创建时间：2018年5月22日 项目名称：ord-api
 * 
 * @author daniel
 * @version 1.0
 * @since JDK 1.8 文件名称：OrderSignInDic.java 类说明： 订单取消返回值字典
 */
public enum OrderCancelDic implements EnumBase {
     ...
     */
    @JsonCreator
    public static OrderCancelDic getItem(final int code) {
        final EnumBase result = valueMap.get(code);
        if (result == null) {
            throw new IllegalArgumentException("输入的code" + code + "不在枚举的取值范围内");
        }
        return (OrderCancelDic) result;
    }
