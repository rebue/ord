...
public class OrderSettleTasks {
    ...
    @Scheduled(fixedDelayString = "${ord.scheduler.settleFixedDelay:300000}")
    public void executeTasks() {
        _log.info("定时执行需要订单结算的任务");
        try {
            _log.info("获取所有需要执行订单结算的任务列表");
            final List<Long> taskIds = taskSvc.getTaskIdsThatShouldExecute(TaskExecuteStateDic.NONE, OrderTaskTypeDic.SETTLE);
