...
public class OrdGoodsBuyRelationSvcImpl extends MybatisBaseSvcImpl<OrdGoodsBuyRelationMo, java.lang.Long, OrdGoodsBuyRelationMapper> implements OrdGoodsBuyRelationSvc {
    ...
    public int add(final OrdGoodsBuyRelationMo mo) {
...
//    @Override
//    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
//    public int addEx(final OrdGoodsBuyRelationMo mo) {
...
//        final OrdGoodsBuyRelationMo goodsBuyRelationMo = _mapper.getByDownlineUserIdAndOnlineId(mo.getDownlineUserId(), mo.getOnlineId());
...
//    }
    ...
    public void exportGoodsBuyRelation() {
        ...
        for (final Entry<String, String> item : listByWildcard.entrySet()) {
            ...
            final String keys = key.replace("rebue.suc.svc.user.buy_relation.", "");
            _log.info("替换前缀之后的值为：{}", keys);
            // 获取用户Id（下家Id）
            final String userId = keys.substring(0, 18);
            _log.info("获取到的用户Id（下家Id）为：{}", userId);
            // 获取上线Id
            final String onlineId = keys.substring(18, 36);
            _log.info("获取到的上家Id为：{}", onlineId);
        }
    }

    @Override
    public OrdGoodsBuyRelationMo getBuyRelation(final OrdGoodsBuyRelationMo mo) {
