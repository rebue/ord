...
public class OrdSettleTaskSvcImpl implements OrdSettleTaskSvc {
     ...
     *            延迟时间
     */
    private void addSettleSubTask(final Long orderId, final Date now, final SettleTaskTypeDic taskType, final Integer delay) {
        final Calendar calendar = Calendar.getInstance();
    ...
    public void executeSettleTask(final OrdTaskMo taskMo) {
        ...
        for (final OrdOrderDetailMo orderDetail : orderDetailList) {
            ...
            case SETTLE_PROFIT_TO_SELLER: {
                ...
                final BigDecimal profitAmount = orderDetail.getActualAmount().subtract(costPriceTotal).subtract(orderDetail.getCashbackTotal()).subtract(platformServiceFee)
            ...
            case SETTLE_PLATFORM_SERVICE_FEE: {
                ...
                // 平台服务费 = 实际成交金额 * 平台服务费比例
                final BigDecimal platformServiceFee = orderDetail.getActualAmount().multiply(platformServiceFeeRatio).setScale(4, BigDecimal.ROUND_HALF_UP);
