...
//	@Override
//	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
//	public String matchBuyRelation(final long userId, final long onlineId, final BigDecimal buyPrice,
//			final long downLineDetailId, final long downLineOrderId, final long orderTimestamp) {
//		_log.info("按匹配自己匹配购买关系");
//		final boolean getBuyRelationResultByOwn = selfSvc.getAndUpdateBuyRelationByOwn(userId, buyPrice,
...
//			final boolean getRegRelationResultByPromote = selfSvc.getAndUpdateBuyRelationByPromote(userId, onlineId,
...
//				final boolean getAndUpdateBuyRelationByInvite = selfSvc.getAndUpdateBuyRelationByInvite(userId,
...
//					final boolean getOtherRelationResultByFour = selfSvc.getAndUpdateBuyRelationByFour(userId, onlineId,
//							buyPrice, downLineDetailId, downLineOrderId);
...
//						final boolean getAndUpdateBuyRelationByFive = selfSvc.getAndUpdateBuyRelationByFive(userId,
//								onlineId, buyPrice, downLineDetailId, downLineOrderId);
...
//							final boolean getOtherRelationResultBySix = selfSvc.getAndUpdateBuyRelationByFive(userId,
//									onlineId, buyPrice, downLineDetailId, downLineOrderId);
...
//	}
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public String matchBuyRelation(final long userId,long InviterUserId,final BigDecimal buyPrice, final long detailId,
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean matchOwnOrInviter(final long userId,final long InviterUserId,final BigDecimal buyPrice, final long detailId,
			final long orderId, final long orderTimestamp) {
		_log.info("匹配购买关系获取订单详情的参数userId-{},InviterUserId-{},buyPrice-{},detailId-{},orderId-{},orderTimestamp-{}", userId,InviterUserId, buyPrice,
				detailId, orderId, orderTimestamp);
		if(InviterUserId==0l) {
		...
		OrdOrderDetailMo UplineOderDetailResult = ordOrderDetailSvc.getCorrectMatchOrderDetail(map);
		...
		ordBuyRelationMo.setUplineOrderId(UplineOderDetailResult.getOrderId());
		ordBuyRelationMo.setUplineUserId(UplineOderDetailResult.getUserId());
		ordBuyRelationMo.setUplineOrderDetailId(UplineOderDetailResult.getId());
		ordBuyRelationMo.setDownlineUserId(userId);
		ordBuyRelationMo.setDownlineOrderDetailId(detailId);
		ordBuyRelationMo.setDownlineOrderId(orderId);
		...
		_log.info("更新上家订单详情返佣名额和返拥金状态的参数为:{}", ordBuyRelationMo);
		final int updateOrderDetailResult = ordOrderDetailSvc.modify(updateOrderDetailMo);
		...
		_log.info("根据规则购买关系成功，匹配的购买关系ID为:{}" + ordBuyRelationMo.getId());
	 ...
	 */
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationByInviter(final long userId, final long  InviterUserId,final BigDecimal buyPrice, final long detailId,
			final long orderId, final long orderTimestamp) {
	...
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationByOwn1(final long id, final long onlineId, final BigDecimal buyPrice,
			final long downLineDetailId, final long downLineOrderId, final long orderTimestamp) {
		// 获取用户购买关系
		...
		final OrdOrderDetailMo orderDetailOfOneCommissionSlot = ordOrderDetailSvc
				.getOrderDetailForOneCommissonSlot(map);
		...
		if (orderDetailOfOneCommissionSlot == null) {
			...
			if (relationResult.size() != 0 && relationResult.get(0).getRelationSource() != null
				...
				final int updateOrderDetailResult = ordOrderDetailSvc
						.updateCommissionSlotForBuyRelation(updateOrderDetailMo);
				if (updateOrderDetailResult != 1) {
	...
	/**
	 * 根据邀请购买规则匹配购买关系 1：根据商品购买关系，获取上家用户是否还有符合规则的商品(上线id相同，价格相同。)
	 */
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationByPromote(final long id, final long onlineId, final BigDecimal buyPrice,
			final long downLineDetailId, final long downLineOrderId) {
		// 获取用户购买关系
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationByInvite(final long id, final long onlineId, final BigDecimal buyPrice,
			final long downLineDetailId, final long downLineOrderId) {
		// 获取用户购买关系
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationByFour(final long id, final long onlineId, final BigDecimal buyPrice,
			final long downLineDetailId, final long downLineOrderId) {
		// 获取用户购买关系
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationByFive(final long id, final long onlineId, final BigDecimal buyPrice,
			final long downLineDetailId, final long downLineOrderId) {
		// 获取用户购买关系
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public boolean getAndUpdateBuyRelationBySix(final long id, final long onlineId, final BigDecimal buyPrice,
			final long downLineDetailId, final long downLineOrderId) {
		// 获取用户购买关系
