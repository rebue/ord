		...
		for (int i = 0; i < orderList.size(); i++) {
			...
			if (subjectType == 0) {
				...
				for (int j = 0; j < buyCount; j++) {
					...
					boolean getBuyRelationResult = getAndUpdateBuyRelation(id, onlineId,
							orderList.get(j).getSalePrice(), detailMo.getId(), detailMo.getOrderId());
					_log.info("获取用户购买关系的返回值为：{}" + getBuyRelationResult);
					if (getBuyRelationResult == false) {
						_log.info("获取用户注册关系上家");
						boolean getRegRelationResult = getAndUpdateRegRelation(id, onlineId,
								orderList.get(j).getSalePrice(), detailMo.getId(), detailMo.getOrderId());
