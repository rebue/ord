...
public class OrdOrderSvcImpl extends MybatisBaseSvcImpl<OrdOrderMo, java.lang.Long, OrdOrderMapper>
	...
	public ShipmentConfirmationRo shipmentConfirmation(final ShipmentConfirmationTo to) {
		...
		if (to.isFirst()) {
			...
			if (to.getAllDetaile().size() == to.getSelectDetaile().size()) {
				_log.debug("需要修改订单状态，订单所有未发货详情等于被选择的详情：AllDetaile长度-{}, SelectDetaile长度-{}",
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public ShipmentConfirmationRo splitPackageDeliver(ShipmentConfirmationTo to) {
		final ShipmentConfirmationRo confirmationRo = new ShipmentConfirmationRo();
		_log.info("将订单中的每个详情发一个包裹并发货的参数为：{}", to);
		_log.info("发货的参数为：{}", to);
		// 判断是否是首次发货还是添加物流单号
		if (to.isFirst()) {
			_log.info("是首次发货：isFirst()-{}", to.isFirst());
		...
		String printPage="";
		for (int i = 0; i < to.getSelectDetaile().size(); i++) {
			_log.info("根据每条详情循环调用快递鸟开始-------------------------------");
			eoderTo.setShipperId(to.getShipperId());
			eoderTo.setShipperCode(to.getShipperCode());
			eoderTo.setOrderId(mo.getId());
			eoderTo.setOrderDetail(to.getSelectDetaile().get(i).getOnlineTitle()+"."+to.getSelectDetaile().get(i).getSpecName()+"."
			+to.getSelectDetaile().get(i).getBuyCount()+"x"+to.getSelectDetaile().get(i).getBuyPrice());
			eoderTo.setOrderTitle(mo.getOrderTitle());
			eoderTo.setReceiverName(mo.getReceiverName());
			eoderTo.setReceiverProvince(mo.getReceiverProvince());
			eoderTo.setReceiverCity(mo.getReceiverCity());
			...
			eoderTo.setOrgId(to.getOrgId());
			_log.info("调用快递电子面单的参数为：{}", eoderTo);
			eOrderRo = kdiSvc.eorder(eoderTo);
			if (eOrderRo.getResult().getCode() == -1) {
				_log.error("调用快递电子面单出现参数错误");
				throw new RuntimeException("调用快递电子面单参数错误");
			}
			...
			if (to.isFirst()) {
				...
				printPage+=eOrderRo.getPrintPage();
				printPage+="<br>\r\n <div style=\"page-break-after:always\"></div>\r\n";
			_log.info("根据每条详情循环调用快递鸟结束++++++++++++++++++++++++++++++++");
		}
		eOrderRo.setPrintPage(printPage);
		_log.info("以下是批量打印的页面↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓");
		_log.info(printPage);
		_log.info("以上是批量打印的页面↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑");
		
		// 判断是否是首次发货还是添加物流单号,不是首次不用修改状态
		if (to.isFirst()) {
			// 根据没有当前订单详情的所有未详情Id和当前被选择的详情Id长度是否相等来决定是否修改订单状态为已发货。
			if (to.getAllDetaile().size() == to.getSelectDetaile().size()) {
				_log.debug("需要修改订单状态，订单所有未发货详情等于被选择的详情：AllDetaile长度-{}, SelectDetaile长度-{}", to.getAllDetaile().size(),
