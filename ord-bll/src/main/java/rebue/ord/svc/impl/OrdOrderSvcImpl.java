...
public class OrdOrderSvcImpl extends MybatisBaseSvcImpl<OrdOrderMo, java.lang.Long, OrdOrderMapper>
	...
	public Ro modifyPayOrderId(final Long id) {
		...
		for (final OrdOrderDetailMo ordOrderDetailMo : detailList) {
			...
			if (ordOrderDetailMo.getSupplierId() != onlOnlineMo.getSupplierId()) {
				_log.info("根据订单id修改支付订单id修改供应商id的参数为：id={}, newSupplierId={}, oldSupplierId={}",
						ordOrderDetailMo.getId(), onlOnlineMo.getSupplierId(), ordOrderDetailMo.getSupplierId());
				final int updateSupplierIdByIdResult = orderDetailSvc.updateSupplierIdById(ordOrderDetailMo.getId(),
						onlOnlineMo.getSupplierId(), ordOrderDetailMo.getSupplierId());
	...
	public PageInfo<OrdOrderRo> listOrderTrade(final ListOrderTo to, final int pageNum, final int pageSize) {
		...
		_log.info("orderList: ro-{}; pageNum-{}; pageSize-{}", to, pageNum, pageSize);
		final PageInfo<OrdOrderRo> result = PageHelper.startPage(pageNum, pageSize)
				.doSelectPageInfo(() -> _mapper.listOrderTrade(to));
		return result;
	}
	...
	@Override
	public List<OrdOrderDetailMo> selectCommission() {
		Date now = new Date();
		List<OrdOrderDetailMo> result = _mapper.selectCommission();
		for (OrdOrderDetailMo orderDetail : result) {
			try {
				handleCommission(orderDetail, orderDetail.getUserId(), now);
			} catch (Exception e) {
				_log.error("执行出现错误，{}", e);
			}
		}
	...
	private void handleCommission(final OrdOrderDetailMo orderDetail, final Long buyerId, final Date now) {
		...
		_log.error("应该处理的订单详情: {}", orderDetail);
