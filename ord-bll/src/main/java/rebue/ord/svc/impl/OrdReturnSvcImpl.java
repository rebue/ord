...
public class OrdReturnSvcImpl extends MybatisBaseSvcImpl<OrdReturnMo, java.lang.Long, OrdReturnMapper> implements OrdReturnSvc {
    ...
    public Ro addReturn(final AddReturnTo to) {
        ...
        final Ro ro = new Ro();
        if (to.getOrderId() == null || to.getOrderDetailId() == null || to.getReturnCount() == null || to.getReturnType() == null || to.getReturnReason() == null
                || to.getApplicationOpId() == null) {
    ...
    public Ro agreeRefund(final OrdOrderReturnTo to) {
        ...
        _log.info("同意退款修改订单详情实际金额的参数为：订单详情id-{}，新的实际金额-{}，旧的实际金额-{}", to.getOrderDetailId(), detail.getActualAmount().subtract(currentRefundTotal), detail.getActualAmount());
        final int modifyActualAmountResult = orderDetailSvc.modifyActualAmount(to.getOrderDetailId(), detail.getActualAmount().subtract(currentRefundTotal),
    ...
    public Ro receivedAndRefunded(final ReceivedAndRefundedTo to) {
        ...
        final Ro ro = new Ro();
        if (to.getId() == null || to.getIp() == null || to.getOpId() == null
