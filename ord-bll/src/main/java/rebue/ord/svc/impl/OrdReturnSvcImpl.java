...
public class OrdReturnSvcImpl extends MybatisBaseSvcImpl<OrdReturnMo, java.lang.Long, OrdReturnMapper>
	...
	public Ro addReturn(final AddReturnTo to) {
		...
		OrdOrderMo ordOrderMo = orderSvc.getById(to.getOrderId());
		...
		// 签收七天后时间戳
		Long receivedTimestamp = ordOrderMo.getReceivedTime().getTime() + 86400000 * 7;
		// 当前时间戳
		...
		OrdOrderDetailMo ordOrderDetailMo = orderDetailSvc.getById(to.getOrderDetailId());
		...
		// 实际购买数量
		int realBuyCount = ordOrderDetailMo.getBuyCount() - ordOrderDetailMo.getReturnCount();
		...
		int addReturnResult = thisSvc.add(returnMo);
		...
		String[] returnPicPath = to.getReturnPicPath().split(",");
		for (int i = 0; i < returnPicPath.length; i++) {
			OrdReturnPicMo ordReturnPicMo = new OrdReturnPicMo();
			ordReturnPicMo.setId(_idWorker.getId());
			ordReturnPicMo.setPicPath(returnPicPath[i]);
			ordReturnPicMo.setReturnId(returnId);
			_log.info("添加用户退货信息添加退货图片的参数为：{}", ordReturnPicMo);
			int addReturnPicResult = ordReturnPicSvc.add(ordReturnPicMo);
		...
		int modifyReturnStateByIdResult = orderDetailSvc.modifyReturnStateById(to.getOrderDetailId(),
	...
	public PageInfo<OrdReturnTo> selectReturnPageList(final OrdReturnTo qo, final int pageNum, final int pageSize) {
		...
		final PageInfo<OrdReturnTo> result = PageHelper.startPage(pageNum, pageSize)
	 ...
	 * 拒绝退货
	 * 
	 * 1、判断退货编号、订单编号、订单详情ID、拒绝用户编号、拒绝原因等参数是否已传过来 2、查询根据退货编号和订单编号查询退货信息
	 * ，判断该退货编号是否存在和查询退货订单的申请状态是否处于待审核状态 3、查询订单信息并判断订单状态是否处于已取消状态
	 * 4、根据订单编号和订单详情ID查询订单详情信息并判断该订单详情的状态是否处于已退货的状态 5、修改订单详情退货数量和状态 6、修改退货订单信息
	 */
	...
	public RejectReturnRo rejectReturn(final OrdReturnTo record) {
		...
		final OrdOrderMo orderList = orderSvc.getById(orderId);
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED)
	public Ro agreeReturn(AgreeReturnTo to) {
		...
		int returnApproveResult = _mapper.returnApprove(returnMo);
	...
	public Ro receivedAndRefunded(final ReceivedAndRefundedTo to) {
		...
		OrdOrderMo ordOrderMo = orderSvc.getById(ordReturnMo.getOrderId());
		...
		OrdOrderDetailMo ordOrderDetailMo = orderDetailSvc.getById(ordReturnMo.getOrderDetailId());
		...
		int modifyReturnStateByIdResult = orderDetailSvc.modifyReturnStateById(ordReturnMo.getOrderDetailId(),
				returnState);
		...
		Ro refundRo = refundSvc.refund(refundTo);
