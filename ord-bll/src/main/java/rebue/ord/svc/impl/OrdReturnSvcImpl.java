...
public class OrdReturnSvcImpl extends MybatisBaseSvcImpl<OrdReturnMo, java.lang.Long, OrdReturnMapper> implements OrdReturnSvc {
    ...
    public Ro addReturn(final AddReturnTo to) {
        ...
        final Ro ro = new Ro();
        if (to.getOrderId() == null || to.getOrderDetailId() == null || to.getReturnCount() == null || to.getReturnType() == null || to.getReturnReason() == null || to.getApplicationOpId() == null) {
        ...
        if (ordOrderMo.getOrderState().intValue() == OrderStateDic.SIGNED.getCode() || ordOrderMo.getOrderState().intValue() == OrderStateDic.SETTLED.getCode()) {
            // 签收七天后时间戳
        ...
        if (ordOrderMo.getOrderState().intValue() == OrderStateDic.CANCEL.getCode() || ordOrderMo.getOrderState().intValue() == OrderStateDic.ORDERED.getCode()) {
        ...
        if (ordOrderDetailMo.getReturnState().intValue() != ReturnStateDic.NONE.getCode() && ordOrderDetailMo.getReturnState().intValue() != ReturnStateDic.PART_RETURNED.getCode()) {
        ...
        if (to.getReturnPicPath() != null && !to.getReturnPicPath().equals("") && !to.getReturnPicPath().equals("null")) {
        ...
        final int modifyReturnStateByIdResult = orderDetailSvc.modifyReturnStateById(to.getOrderDetailId(), (byte) ReturnStateDic.RETURNING.getCode());
     ...
     */
    @Override
    public PageInfo<ReturnPageListRo> selectReturnPageList(final ReturnPageListRo qo, final int pageNum, final int pageSize) {
        _log.info("selectReturnPageList: qo-{}; pageNum-{}; pageSize-{}", qo, pageNum, pageSize);
        final PageInfo<ReturnPageListRo> result = PageHelper.startPage(pageNum, pageSize).doSelectPageInfo(() -> _mapper.selectReturnPageList(qo));
    ...
    public Ro rejectReturn(final RejectReturnTo to) {
        ...
        if (ordReturnMo.getApplicationState().intValue() != ReturnApplicationStateDic.RETURNING.getCode() && ordReturnMo.getApplicationState().intValue() != ReturnApplicationStateDic.PENDING_REVIEW.getCode()) {
        ...
        if (orderMo.getOrderState().intValue() == OrderStateDic.CANCEL.getCode() || orderMo.getOrderState().intValue() == OrderStateDic.ORDERED.getCode()) {
        ...
        final int modifyReturnStateByIdResult = orderDetailSvc.modifyReturnStateById(orderDetailMo.getId(), (byte) ReturnStateDic.NONE.getCode());
        _log.info("添加用户退货信息修改订单详情退货状态的返回值为：{}", modifyReturnStateByIdResult);
        if (modifyReturnStateByIdResult != 1) {
    ...
    public Ro agreeRefund(final OrdOrderReturnTo to) {
        ...
        _log.debug("检查参数的正确性");
        if (// 
        to.getReturnId() == null || to.getOrderId() == null || to.getOrderDetailId() == null || to.getOpId() == null || (to.getRefundAmount() == null && (to.getRefundAmount1() == null || to.getRefundAmount2() == null))) {
        ...
        if (order.getOrderState() == OrderStateDic.CANCEL.getCode() || order.getOrderState() == OrderStateDic.ORDERED.getCode() || order.getOrderState() == OrderStateDic.SETTLED.getCode()) {
        ...
        if (ReturnApplicationStateDic.PENDING_REVIEW.getCode() != returnInfo.getApplicationState() && ReturnApplicationStateDic.RETURNING.getCode() != returnInfo.getApplicationState()) {
        ...
        // 新订单详情实际成交金额
        BigDecimal newActualAmount = detail.getActualAmount().subtract(currentRefundTotal).setScale(4, BigDecimal.ROUND_HALF_UP);
        ...
        // 修改订单退款金额
        _log.info("修改订单退款金额的参数为：refundTotal={}, orderState={}, refundedTotal={}", refundTotal, orderState, refundedTotal);
        ...
        if (to.getReturnNum() != null && to.getReturnNum() > 0) {
            ...
            _log.info("同意退款修改返现总额和退货数量的参数为：id={}, oldCashbackTotal={}, newCashbackTotal={}, returnedCount={}, returnTotal={}", to.getOrderDetailId(), oldCashbackTotal, newCashbackTotal, returnedCount, returnTotal);
            int modifyReturnNumAndCashbackTotalResult = orderDetailSvc.modifyReturnNumAndCashbackTotal(to.getOrderDetailId(), oldCashbackTotal, newCashbackTotal, returnedCount, returnTotal);
        ...
        _log.info("同意退款修改订单详情实际金额和退货状态的参数为：订单详情id-{}，新的实际金额-{}，旧的实际金额-{}", to.getOrderDetailId(), detail.getActualAmount().subtract(currentRefundTotal), detail.getActualAmount());
        final int modifyActualAmountResult = orderDetailSvc.modifyActualAmountANDReturnState(to.getOrderDetailId(), detail.getActualAmount().subtract(currentRefundTotal), detail.getActualAmount(), returnState, detail.getReturnState());
        ...
        // 
        // 
        _log.info("确认退款修改退货单的参数为：本次退款总额-{} 扣除补偿金额-{} 操作人ID-{} 操作时间-{} 退货ID-{}", currentRefundTotal, to.getRefundCompensation(), to.getOpId(), now, to.getReturnId());
        final int // 
        confirmRefundRowCount = _mapper.confirmRefund(currentRefundTotal, to.getRefundCompensation(), (byte) ReturnApplicationStateDic.TURNED.getCode(), to.getOpId(), now, to.getReturnId());
        ...
        if ((newActualAmount.compareTo(BigDecimal.ZERO) == 0) && order.getOrderState() == OrderStateDic.PAID.getCode()) {
            _log.info("同意退款根据上线规格id修改销售数量的参数为：onlineSpecId-{}. buycount-{}", detail.getOnlineSpecId(), detail.getBuyCount());
            Ro modifySaleCountByIdResult = onlOnlineSpecSvc.modifySaleCountById(detail.getOnlineSpecId(), detail.getBuyCount());
    ...
    public Ro receivedAndRefunded(final ReceivedAndRefundedTo to) {
        ...
        final Ro ro = new Ro();
        if (to.getId() == null || to.getIp() == null || to.getOpId() == null || (to.getRefundAmount() == null && to.getReturnBalanceToBuyer() == null && to.getReturnCashbackToBuyer() == null)) {
        ...
        if (to.getRefundAmount() != null) {
            ...
            refundTotal = refundCompensation.add(to.getReturnBalanceToBuyer()).add(to.getReturnCompensationToSeller()).setScale(4, BigDecimal.ROUND_HALF_UP);
        }
        returnMo.setId(to.getId());
        ...
        Byte returnState = (byte) ReturnStateDic.PART_RETURNED.getCode();
        // 订单详情真实购买数量
        final int realBuyCount = ordOrderDetailMo.getBuyCount() - ordOrderDetailMo.getReturnCount() - ordReturnMo.getReturnCount();
        if (realBuyCount <= 0) {
            returnState = (byte) ReturnStateDic.RETURNED.getCode();
        }
        _log.info("已收到货并退款修改订单详情退货状态的参数为：{}", ordReturnMo.getOrderDetailId());
        final int modifyReturnStateByIdResult = orderDetailSvc.modifyReturnStateById(ordReturnMo.getOrderDetailId(), returnState);
     ...
     */
    @Override
    public List<Map<String, Object>> selectReturningInfo(final Map<String, Object> map) throws ParseException, IntrospectionException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    @Override
    public List<Map<String, Object>> selectReturnInfo(final Map<String, Object> map) throws ParseException, IntrospectionException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
