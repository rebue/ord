...
@Service
public class OrdReturnSvcImpl extends
        BaseSvcImpl<java.lang.Long, OrdReturnJo, OrdReturnDao, OrdReturnMo, OrdReturnMapper> implements OrdReturnSvc {
...
//	private OrdBuyRelationSvc ordBuyRelationSvc;
     ...
     *
     * 同意退款流程：
     * 1、根据订单id查询订单信息并判断订单是否存在、订单状态是否处于可退货的状态（已支付、已发货、已签收）
     * 2、根据订单详情id查询订单详情id并判断订单详情是否存在、退货状态是否处于退货中
     * 3、根据退货id查询退货单信息并判断退货单是否存在、是否处于待审核或退货中
     * 4、判断当前退款方式是否为自定义退款，如果为自定义退款则计算本次退款金额（退款金额1 + 退款金额2）
     * 5、计算本次退款总额（本次退款金额 + 扣除补偿金额）、新的订单退款总额（订单已退款总额 +
     * 本次退款总额）并判断新的订单退货总额是否大于订单的实际支付金额
     * 6、判断新订单总额是否等于订单实际支付金额，如果等于则设置订单状态为取消状态
     * 7、修改订单退款金额（如果订单状态不为null则同步修改订单状态）
    ...
    public Ro agreeRefund(final OrdOrderReturnTo to) {
        ...
        if (detail.getSubjectType() == 1) {
            ...
            addTaskMo.setOrderDetailId(detail.getId());
            _log.info("添加重新匹配购买关系的参数为:{}", addTaskMo);
