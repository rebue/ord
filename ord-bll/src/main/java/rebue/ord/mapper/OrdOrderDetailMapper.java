...
public interface OrdOrderDetailMapper extends MybatisBaseMapper<OrdOrderDetailMo, Long> {
     ...
     */
    @//
    Select("SELECT " + "    IFNULL(SUM(a.BUY_COUNT - a.RETURN_COUNT),0) " + " FROM" + "    ORD_ORDER_DETAIL AS a," + "    ORD_ORDER AS b " + " WHERE" + "    a.ORDER_ID = b.ID"
    ...
    int updateReturnNumAndCashbackTotal(@Param("id") Long id, @Param("oldCashbackTotal") BigDecimal oldCashbackTotal, @Param("newCashbackTotal") BigDecimal newCashbackTotal,
            @Param("returnedCount") Integer returnedCount, @Param("returnTotal") Integer returnTotal);
     ...
     *            支付顺序
     */
    @Update("UPDATE ORD_ORDER_DETAIL " + //
            " SET " + //
            "    PAY_SEQ = NULL" + //
            " WHERE" + //
