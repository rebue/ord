...
public interface OrdReturnMapper extends MybatisBaseMapper<OrdReturnMo, Long> {
    ...
    // 
    int confirmRefund(@Param("refundTotal") BigDecimal refundTotal, @Param("refundCompensation") BigDecimal refundCompensation, @Param("applicationState") Byte applicationState, @Param("opId") Long opId, @Param("opTime") Date opTime, @Param("id") Long returnId);
     ...
     */
    @// 
    Select("SELECT " + "    COUNT(*) > 0" + " FROM" + "    ORD_RETURN" + " WHERE" + "    ORDER_ID = #{orderId,jdbcType=BIGINT}" + "        AND APPLICATION_STATE IN (1 , 2)")
